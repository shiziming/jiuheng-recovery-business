...
<style>
#publish_sku_page .main_div{
	float: left;
	width: 60px;
	height: 60px;
	margin: 10px
}
#publish_sku_page .main_imgs{
	width: 55px;
	height: 55px
}
#publish_sku_page .detail_imgs{
	width: 250px;
	height: auto;
}
#publish_sku_page .main_div_hover{
	border : 2px solid #e4393c
}
</style>
<div id="publish_sku_page" style="width: 100%;">
	<div align="center">
		...
		<a style="float: right" class="easyui-linkbutton" data-options="iconCls:'icon-back'"
				onclick="spuInfo.returnToListPage();"> <span>返 回</span></a>
		</p>
	</div>
		...
		<fieldset>
			...
			<div style="padding: 10px 20px">
				<a href="javascript:void(0);" id="btnSave" class="easyui-linkbutton"
					...
					style="padding: 5px 0px; width: 150px;" onclick="spu.publish();">
...
<script>
	$("#publish_sku_page .main_div").mouseover(function(){
		$(this).addClass('main_div_hover');
		$("#publish_sku_page #img_big_div").attr('src',$(this).find('img').attr('src'));
	});
	$("#publish_sku_page .main_div").mouseout(function(){
			...
			success : function(result){
				if(0 == result.code ){
					$.each(result.data,function(i,data){
						...
						if(1 == data.location){
							main_pic = 1;
							var li = $('#publish_sku_page').find('#main_pic').find('div[data-id=1_'+data.inx+']');
							...
							$('#publish_sku_page #mat-detail').append(div);
						}
					});
				}
			}
