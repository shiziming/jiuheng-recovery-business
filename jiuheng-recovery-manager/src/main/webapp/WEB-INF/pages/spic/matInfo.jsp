...
<style>
#material_info_1 .main_div{
	float: left;
	width: 60px;
	height: 60px;
	margin: 10px
}
#material_info_1 .main_imgs{
	width: 55px;
	height: 55px
}
#material_info_1 .detail_imgs{
	width: 250px;
	height: auto;
}
#material_info_1 .main_div_hover{
	border : 2px solid #e4393c
}
</style>
<div id="material_info_1" style="width: 100%;">
	<div align="center">
...
<script>
	$("#material_info_1 .main_div").mouseover(function(){
		$(this).addClass('main_div_hover');
		$("#material_info_1 #img_big_div").attr('src',$(this).find('img').attr('src'));
	});
	$("#material_info_1 .main_div").mouseout(function(){
			...
			success : function(result){
				if(result.code != undefined && result.code == 0){
					$.each(result.data,function(i,data){
						...
						if(1 == data.location){
							var li = $('#material_info_1').find('#main_pic').find('div[data-id=1_'+data.inx+']');
							...
							$('#material_info_1 #mat-detail').append(div);
						}
					});
				}
			}
