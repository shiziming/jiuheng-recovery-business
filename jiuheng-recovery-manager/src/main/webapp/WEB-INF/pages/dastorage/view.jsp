...
<div id="dastorage_view_page">
<form id="form" method="post">
            <input id="billId" type="hidden" value="${daStorageSetBill.billId}">
			...
			<table border=1
				...
				<tr>
					...
					<td width="25%">${daStorageSetBill.billId}</td>
					<td width="25%" align="center">公司代码</td>
					<td width="25%">${daStorageSetBill.companyCode}</td>
				</tr>
				<tr>
					<td width="25%" align="center">状态</td>
					<c:if test="${daStorageSetBill.status==1}">
						<td width="25%">已审核</td>
					</c:if>
					<c:if test="${daStorageSetBill.status==0}">
						<td width="25%">已录入</td>
					</c:if>
					...
					<td width="25%">${daStorageSetBill.billOrgCode}</td>
				</tr>
				<tr>
					<td width="25%" align="center">录入人</td>
					<td width="25%">${daStorageSetBill.createName}</td>
					<td width="25%" align="center">录入时间</td>
					<td width="25%">${daStorageSetBill.createTime}</td>
				</tr>
				<c:if test="${status!=0}">
					<tr>
						<td width="25%" align="center">审核人</td>
						<td width="25%">${daStorageSetBill.checkName}</td>
						<td width="25%" align="center">审核时间</td>
						<td width="25%">${daStorageSetBill.checkTime}</td>
					</tr>
				</c:if>
			</table>
		</form>
