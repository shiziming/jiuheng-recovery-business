...
<div id="amazonOrder_view" style="width: 100%; height: auto;">
		...
		<div id="order_panel" class="easyui-panel" title="主单信息"	style="width: 100%; background-color: #eff5ff;"
			data-options="collapsible:true">
				<table class="table table-hover table-condensed" style="width: 100%; padding: 3px 1% 3px 1%; ">
					<tr>
						<td width="30px" align="right" style="padding-left: 30px">亚马逊订单编号 :</td>
					...
					<tr>
						...
						<td id="shrdh1">${address.phone1}<br>${address.phone2}</td>
					...
					<tr>
						...
						<td ><fmt:formatNumber type="number" value="${(mainOrder.ddpsf)/100}"  pattern="0.00" maxFractionDigits="2"/></td>
					</tr>
			...
			data-options="collapsible:true">
				<table class="table table-hover table-condensed" style="width: 100%; padding: 3px 1% 3px 1%;">
					<tr>
						...
						<td width="30px" align="right">订单发货时间:</td>
					...
					<tr>	
						...
						<td id="fdxh" width="50px"><fmt:formatNumber type="number" value="${(branchOrder.ddpsf)/100}"  pattern="0.00" maxFractionDigits="2"/></td>
					...
					<tr>
						...
						<c:choose>
						   <c:when test="${branchOrder.yxsfdh>0}">
						    <td width="50px"><a href="#" onclick="amazonOrderview.view(${branchOrder.yxsfdh});">${branchOrder.yxsfdh}</a></td>
						   </c:when>
					...
					<tr>
						...
						<td width="30px" align="right">地址配置的实际库存地 :</td>
					...
					<tr>
					 <td width="30px" align="right">在途预售标记 :</td>
					<td id="spkclx" width="50px">
						<c:if test="${branchOrder.spkclx==1}">门店</c:if>
						<c:if test="${branchOrder.spkclx==2}">在库</c:if>
						<c:if test="${branchOrder.spkclx==3}">在途</c:if>
						<c:if test="${branchOrder.spkclx==4}">带安</c:if>
						</td>
					    <td width="50px">物流确定配送日期</td>
						<td width="30px" align="right">${address.deliveryDetermineDate}</td>
						<td width="50px"></td>
	        ...
	        <table cellspacing="0" cellpadding="0" width="100%" style="border-bottom:1px solid #95B8E7" class="pn-ltable">
				<thead class="pn-lthead">
					<tr  bgcolor="#DCDCDC">
						...
						<td width="4%" style="background-color:#F4F4F4;font-size:12px;padding:3px;border-right:1px dotted #CCCCCC;">亚马逊商品识别号</td>
						<td width="4%" style="background-color:#F4F4F4;font-size:12px;padding:3px;border-right:1px dotted #CCCCCC;">亚马逊订单商品识别号</td>
...
<!-- 		overflow-x : scroll; -->
		 	<div class="easyui-panel" title="订单履历" data-options="border:false,collapsible:true" style="width: 100%;min-height: 100px;background-color: #eff5ff;">
				...
				collapsible:true,
				url:'amazonOrder/queryOrderStatusByXsfdh/'+${branchOrder.xsfdh},
				striped : true,
				autoRowHeight:true,
				checkOnSelect : true,
				singleSelect : true">
...
<!-- 							<th data-options="field:''">承运商信息</th> -->
		...
		<div style="width: 100%;padding: 5px 0px;background-color: #eff5ff;" align="center">
			<a href="javascript:void(0);" id="btnReturn"
				...
				onclick="amazonOrderview.Return();"> <span
				style="font-size: 14px;">返 回</span>
			</a>
	   </div>
 </div>
</div>
<script type="text/javascript"	src="common/js/amazonOrder/amazonOrderView.js"></script>
