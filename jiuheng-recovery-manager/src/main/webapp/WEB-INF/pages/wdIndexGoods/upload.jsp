...
<script type="text/javascript">
	var	orderNum=$('#machine_uploader #orderNum').val()
	...
	indexGoods_detailed.uploader = WebUploader.create({
		auto : false,
		method : 'POST',
		swf : ctx + '/js/webuploader/Uploader.swf',
		server : ctx + '/wdIndexGoodsController/uploadIndexGoodsImage/'+orderNum,
	...
	indexGoods_detailed.uploader.on('fileQueued', function(file) {
	...
	indexGoods_detailed.uploader.on('uploadAccept', function(object, ret) {
		var result = jQuery.parseJSON(ret._raw);
		if (result.code == 1) {
			$('#machine_uploader #dgtable').datagrid('updateRow', {
				index : object.file.id.split('_')[2],
				row : {
					status : result.msg
				}
			});
	...
	indexGoods_detailed.uploader.on('error', function(type) {
	...
	$('#machine_uploader #ctlBtn').on('click', function() {
		indexGoods_detailed.uploader.upload();
	});
	function back(){
		$(indexGoods_detailed.update_panel).panel('close');
